...
| 学号人工审核（V13 新增） | **学号（必填）** + 姓名 / 学生证照片（可选） | 管理员在审核队列里通过 / 驳回 | 学校**没有**学生邮箱 |

> 人工通道的门槛刻意压到最低：**只有学校与学号必填**，姓名与学生证照片都是可选加分项。
> 这条通道面向的正是"学校连邮箱都没有"的场景，再要求上传证件照，很可能把同一批学生又挡在门外
> （有人没有学生证照片、有人不愿上传证件）。需要更严格核验时，收紧开关或要求补材料即可，
> 而"可选"不会拦住任何人。
...
1. **学生侧**：任意账号登录 → 个人中心 →「校园身份认证」→ 点底部「没有校园邮箱？用学号认证（人工审核）」→
   选学校、填学号（姓名与照片可选）→ 提交；页面显示"材料已提交，等待管理员审核"。
